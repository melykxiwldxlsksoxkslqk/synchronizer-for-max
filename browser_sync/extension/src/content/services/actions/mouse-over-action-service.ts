...
import type { BrowserActionEnvelope, MouseOverActionPayload } from "../../types";
...
function normalizeMouseOverPayload(raw: unknown): MouseOverActionPayload | null {
...
export class MouseOverActionService extends BrowserActionServiceTemplate {
  readonly actionType = ACTION_TYPES.mouseOver;
  readonly listenedEventTypes = ["mouseover"] as const;

  capture(event: Event): MouseOverActionPayload | null {
  ...
  getMuteKey(payload: unknown): string | null {
    const normalized = normalizeMouseOverPayload(payload);
    return normalized?.fieldKey || null;
  }

  apply(action: BrowserActionEnvelope, context: ActionApplyContext): void {
    const payload = normalizeMouseOverPayload(action.payload);
    if (!payload) return;
    ...
    context.logger.debug(`applied mouseover key=${payload.fieldKey}`);
  }
}
